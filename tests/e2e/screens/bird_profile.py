...
try:
    with sync_playwright() as p:
        ...
        h.evaluate("async () => { const db = await window.__zajilDb; await db.importAll(await (await fetch(new URL('sample-data.json', document.querySelector('link[rel=manifest]').href))).json(), 'merge'); }")
        ...
        pg.goto(f"{ROOT}bird/?id={ids['barq']}", wait_until='load'); pg.wait_for_selector('[data-testid=profile-hero]', timeout=6000)
        ...
        pg.goto(f"{ROOT}bird/?id={racer[0]}&tab=race", wait_until='load'); pg.wait_for_selector('[data-testid=race-best]', timeout=6000)
        ...
        pg.goto(f"{ROOT}bird/?id={ids['barq']}&tab=race", wait_until='load'); pg.wait_for_selector('[data-testid=profile-hero]')
        ...
        pg.goto(f"{ROOT}bird/?id={lone}&tab=hlth", wait_until='load'); pg.wait_for_selector('[data-testid=profile-hero]')
        ...
        pg.goto(f"{ROOT}bird/?id={ids['orphan']}", wait_until='load'); pg.wait_for_selector('[data-testid=profile-hero]')
        ...
        wipe(h); h.evaluate("async () => { const db = await window.__zajilDb; await db.importAll(await (await fetch(new URL('example-loft-large.json', document.querySelector('link[rel=manifest]').href))).json(), 'merge'); }")
        ...
        if rid:
            pg.goto(f"{ROOT}bird/?id={rid}&tab=ped", wait_until='load'); pg.wait_for_selector('[data-testid=progeny]')
        ...
        if asif:
            pg.goto(f'{ROOT}bird/?id={asif}', wait_until='load'); pg.wait_for_selector('[data-testid=tile-coi]', timeout=8000)
        ...
        withphoto = h.evaluate("""async () => { const db = await window.__zajilDb;
            ...
            return b.id; }""")
        leak.goto(f'{ROOT}bird/?id={withphoto}', wait_until='load')
